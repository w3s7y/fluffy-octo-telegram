...
class Address(models.Model):
    SY = "Shropshire"
    CH = "Cheshire"
    ORG_AREA_CHOICES = [(SY, "Shropshire and North Wales"), (CH, "Cheshire and Wirral")]
    ...
    org_area = models.CharField(max_length=100, choices=ORG_AREA_CHOICES, blank=False)
...
class Human(User):
    ...
    home_address = models.ForeignKey('Address', blank=False, on_delete=models.RESTRICT, null=True)
...
class Client(Human):
    def __str__(self):
        return f"{self.first_name} {self.middle_names} {self.last_name} - {self.home_address.post_code}"
...
class Pet(models.Model):
    ...
    species = models.CharField(max_length=200, blank=False, choices=SPECIES_CHOICES, null=True)
...
class Vet(Human):
    ...
    base_surgery = models.ForeignKey('Surgery', on_delete=models.RESTRICT, null=True)
...
class Appointment(models.Model):
    ...
    surgery = models.ForeignKey('Surgery', on_delete=models.RESTRICT, null=True)
    ...
    def __str__(self):
        return f"{self.surgery} - {self.vet} - {self.pet} - {self.appointment_date_time.__str__()}"
